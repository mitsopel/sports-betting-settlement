...
class SportEventOutcomeServiceImplTest {
    ...
    @Test
    void shouldPublishDelegatingToKafkaProducer() {
        SportEventOutcome outcome = new SportEventOutcome("EVT-1", "Event One", "WIN-1");

        doNothing().when(kafkaProducer).publish(outcome);
        service.publish(outcome);

        verify(kafkaProducer, times(1)).publish(outcome);
    }

    @Test
    void shouldHandleFilteringWinnersBuildMessageAndForward() {
        var winner = new BetEntity(1L, 101L, "EVT-1", "MKT-1", "WIN-1", BigDecimal.TEN);
        var loser = new BetEntity(2L, 102L, "EVT-1", "MKT-1", "WIN-2", BigDecimal.valueOf(20));
        ...
        service.handle(new SportEventOutcome("EVT-1", "Event One", "WIN-1"));
